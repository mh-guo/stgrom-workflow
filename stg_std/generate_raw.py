...
handler = STGHandler(EOS_name)

#logAlphas = TSPoints([-5.3, -4.0, -2.5], [26, 15])
#betas = TSPoints([-4.8, -4.6, -4.3, -4.0], [10, 60, 30])
#logAlphas = TSPoints([-5.6, -4.0, -2.2+1e-15], [32, 36])
#betas = TSPoints([-4.8, -4.6, -4.3, -4.0], [10, 60, 30])
logAlphas = TSPoints([-5.6, -4.0, -2.2+1e-15], [3, 4])
betas = TSPoints([-4.8, -4.6, -4.3, -4.0], [1, 6, 3])


#e_cs = TSDensity(EOS_Range[EOS_name], [180, 400, 240, 400, 150])

e_cs = TSDensity(EOS_Range[EOS_name], [18, 40, 24, 40, 15])

print('logAlphas:',logAlphas)
print('betas:',betas)    

handler.iter_GridPara(e_cs=e_cs, logAlphas=logAlphas, betas=betas) 
handler.status()

if os.path.isdir(label) is False:
   os.mkdir(label)

handler.generate(label=label, threading=cpu_count, max_iter=50, verbose=False)
# handler.generate(label=label, threading=cpu_count, max_iter=50, high_s=0.2, verbose=False)
